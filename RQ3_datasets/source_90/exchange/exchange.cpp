...
extern "C" {
	...
	void apply( uint64_t receiver, uint64_t code, uint64_t action ) {
		...
		//apply_onerror( receiver, onerror::from_current_action() );
			...
			switch(action) {
					case N(seteosasset): return exchange::seteosasset(receiver, code);  //3.2
					case N(setadmin): return exchange::setadmin(receiver, code);
					case N(transfer): return exchange::deposit(receiver, code);    //2.1
					case N(makeorder): return exchange::makeorder(receiver, code); //1.2
					case N(takeorder): return exchange::takeorder(receiver, code); //1.1
					case N(cancelorder): return exchange::cancelorder(receiver, code);  //3.1
					case N(makewithdraw): return exchange::makewithdraw(receiver, code);  //2.2
				}
			}
	}
}
